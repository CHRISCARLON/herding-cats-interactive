...
class CatalogButton(Button):
    ...
    def _format_catalog_list(self) -> Text:
        ...
        ckan_catalogs = [
            name for name, (type_, _) in self.catalogs.items() if type_ == "ckan"
        ]
        ods_catalogs = [
            name
            for name, (type_, _) in self.catalogs.items()
            if type_ == "opendatasoft"
        ]
        fr_catalogs = [
            name for name, (type_, _) in self.catalogs.items() if type_ == "french_gov"
        ]
